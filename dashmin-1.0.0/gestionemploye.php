<?php include('phpSessionGesEmp.php');
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <div class="container">
        <form method="post" action="">
            <input type="search" name="motcle" placeholder="Recherche par le Nom">
            <button class=" btn btn-primary my-5" name="btnsubmit">  Rechercher </button>
     ...
     /* if(isset($_POST['btnsubmit']))
    {
        ...
        $req="select * from employe where nom_emp like '$motrechercher%'";
    }
    else
    {
    ...
    $res=mysqli_query($conn,$req);
        ...
        { 
            ?>
            <?php $mat_emp=$ligne['MATRICULE_EMP'] ?>
            <?php echo $ligne['NUMERO_FCT'] ?>
            <?php echo $ligne['MATRICULE_ADM'] ?>
            <?php echo $ligne['CODE_ETIQUETTE'] ?>
            <?php echo $ligne['NOM_EMP'] ?>
            <?php echo $ligne['PRENOM_EMP'] ?>
            <?php echo $ligne['DATE_RECRUTEMENT'] ?>
            <?php echo $ligne['DATE_NAISSANCE'] ?>
            <?php  echo $ligne['LOGIN_EMP'] ?>
            <?php echo $ligne['MDP_EMP'] ?>
            <?php echo $ligne['SALAIRE'] ?>
       <?php } */?> 
    <div class="container">
  ...
  if($result)
  {
        while($row=mysqli_fetch_assoc($result))
        {
            ...
            <td>'.$salaire.'</td>
            <td>
            <a href="UpdateEmp2.php?updateid='.$matriculeemp.'" class="text-light">
            <img src="stylo.png" width=30px>
           
            </td>
            <td>
            <a href="deleteemp.php?deleteid='.$matriculeemp.'" class="text-light">
